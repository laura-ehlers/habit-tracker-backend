...
class Progress(object):
    ...
    def format_periodicity(self, periodicity):
        if (periodicity <= 0) and self.max is None or self.cur == self.min:
            result = "??:??:??"
        # elif periodicity < 1:
        #     result = '--:--:--'
        else:
            result = time.strftime("%H:%M:%S", time.gmtime(periodicity))
        return result
    ...
    def ETA(self):
        if self.done:
            ...
            if self.max is None:
                ...
                t /= self.cur - self.min
                t = (t - 1) * self.elapsed
        return "%s: %s" % (prefix, self.format_periodicity(t))
